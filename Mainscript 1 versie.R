...
databestand <- dlgInput("Wat is de naam van het ruwe data bestand? ", Sys.info()["databestand"])$res
naamtoets <- dlgInput("Wat is de naam van de toets ", Sys.info()["naamtoets"])$res
...
teleformdata <- teleformdata %>%
  select(stud_nr, stud_naam, everything())
    
##Laden van transformatie script
ll <- parse(file = "/Toetsanalyse 1 versie_advies.R")
...
thetitle=naamtoets; rmarkdown::render("/Itemanalyse2.Rmd", output_file = paste0(Network_directory,"Itemanalyse.pdf"))

##Bereken scores per vraaggroep. Pas onderstaand script aan welke vragen bij elkaar horen
newnames <- c("studentnummer", vrn)
colnames(scored_datax) <- newnames
vraag_groep <- mutate(scored_datax, groep1=V1+V2+V3+V4, groep2=V5+V6+V7+V8, groep3=V9+V10+V11+V12)
vraag_groep_score <- select(vraag_groep, studentnummer, groep1, groep2, groep3)
write.csv2(vraag_groep_score, file="vraaggroepen.csv", row.names=FALSE)
