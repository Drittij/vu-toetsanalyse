...
names(stap2$toetswaarden) <- c("Aantal vragen", "Aantal studenten", "Betrouwbaarheid toets (KR20)", "Betrouwbaarheid toets (KR20-75)", "Gemiddelde score", "Standaard Afwijking score", "P'-Gem", "P-Gem", "Percentage geslaagde studenten", "Cesuur", "Livingston's kappa")
toetswaardent <- as.data.frame(t(stap2$toetswaarden))
...
kable(stap2$itemanalyse, digits = 2, "markdown", row.names = F)
...
geom_vline(xintercept = stap2$toetswaarden$`Gemiddelde score`, color="blue")
...
set.seed(42)
ggplot(stap2$itemanalyse, aes(rir, P_waarde)) +
  geom_rect(data=stap2$itemanalyse,aes(xmin=min(rir)-0.1,xmax=0.1,ymin=-Inf,ymax=0.3),fill="red") + 
  geom_rect(data=stap2$itemanalyse,aes(xmin=0.1,xmax=max(rir)+0.1,ymin=0.3,ymax=Inf),fill="lightgreen")+ 
  geom_rect(data=stap2$itemanalyse,aes(xmin=0.1,xmax=max(rir)+0.1,ymin=-Inf,ymax=0.3),fill="orange")+
  geom_rect(data=stap2$itemanalyse,aes(xmin=min(rir)-0.1,xmax=0.1,ymin=0.3,ymax=Inf),fill="orange")+
